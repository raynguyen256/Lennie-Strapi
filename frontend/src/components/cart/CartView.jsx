...
function CartRow({ item, checked, onToggle, onQty, onRemove }) {
  return (
    <div className="flex gap-4 sm:gap-5 py-6 border-b border-divider">
      ...
      <Link href={`/shop/${item.slug}`} className="w-24 h-24 sm:w-28 sm:h-28 shrink-0 bg-brand-blue-light border border-divider rounded-sm flex items-center justify-center p-3 overflow-hidden group">
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <Link href={`/shop/${item.slug}`} className="font-serif text-[17px] sm:text-lg font-medium text-ink leading-snug hover:text-brand-blue transition-colors">
              {item.name}
            </Link>
            <div className="font-sans text-[13px] text-ink-2 mt-1.5">
              {vnd(item.price)} <span className="text-ink-3">/ sản phẩm</span>
            </div>
          </div>
          <button type="button" onClick={() => onRemove(item.id)} className="shrink-0 inline-flex items-center gap-1 text-ink-3 hover:text-brand-blue font-sans text-[11px] font-bold tracking-wider uppercase p-1">
            <Icon.X size={15} />
        ...
        <div className="flex items-end justify-between gap-3 mt-auto pt-4">
          <div className="flex items-center border border-divider rounded-sm">
            <button type="button" onClick={() => onQty(item.id, item.qty - 1)} className="w-10 h-10 flex items-center justify-center text-ink-2 hover:text-brand-blue">
              −
            </button>
      ...
      <section className="bg-white py-20 md:py-28">
        <div className="max-w-2xl mx-auto px-6 text-center flex flex-col items-center gap-5">
          ...
          <p className="font-sans text-sm text-ink-2 leading-relaxed max-w-md">
            Hãy khám phá bộ sưu tập dược mỹ phẩm chọn lọc được ThS. DS. Hoàng Hồng Thắm tuyển chọn và thêm sản phẩm phù hợp với làn da bạn.
          </p>
        ...
        <div className="lg:col-span-7 xl:col-span-8">
          <div className="flex items-center justify-between pb-4 border-b border-divider">
            <h2 className="font-serif text-2xl text-ink font-semibold">
              Sản phẩm <span className="text-ink-3 font-light">({count})</span>
            </h2>
            <button type="button" onClick={clear} className="font-sans text-[10px] font-bold tracking-wider text-ink-3 hover:text-brand-blue uppercase">
              Xóa tất cả
            </button>
          </div>
          ...
          {items.map((it) => (
            <CartRow key={it.id} item={it} checked={isSelected(it.id)} onToggle={toggleSelect} onQty={setQty} onRemove={remove} />
          ))}
          <Link href="/shop" className="inline-flex items-center gap-2 mt-7 font-sans text-[11px] font-bold tracking-[0.16em] uppercase text-brand-blue hover:text-ink transition-colors">
          ...
          <div className="lg:sticky lg:top-28 bg-mist border border-divider rounded-md p-6 md:p-7 space-y-5">
            ...
            <div className="space-y-3 font-sans text-sm">
              <div className="flex justify-between text-ink-2">
                ...
                <span className="font-semibold text-ink">{vnd(selectedSubtotal)}</span>
              </div>
              <div className="flex justify-between text-ink-2">
            ...
            {selectedCount > 0 ? (
              <Link href="/checkout" className="w-full py-4 bg-brand-blue text-white font-sans text-[10px] font-bold tracking-[0.2em] uppercase rounded-sm hover:bg-ink transition-colors flex items-center justify-center gap-2">
                <Icon.ArrowR size={15} />
                Tiến hành thanh toán
              </Link>
            ) : (
              <button type="button" disabled className="w-full py-4 bg-divider text-ink-3 font-sans text-[10px] font-bold tracking-[0.2em] uppercase rounded-sm flex items-center justify-center gap-2 cursor-not-allowed">
                <Icon.ArrowR size={15} />
                Tiến hành thanh toán
              </button>
            )}
            <div className="bg-white border border-divider rounded-md p-4 flex items-start gap-3">
              ...
              <p className="font-sans text-[12px] text-ink-2 leading-relaxed">
                Sau khi đặt hàng, <strong className="text-ink">nhân viên Lennie</strong> sẽ liên hệ để xác nhận đơn và hỗ trợ bạn hoàn tất thanh toán cuối cùng.
              </p>
            </div>
            <div className="grid grid-cols-3 gap-2 pt-1">
              {[
                ["Shield", "Chính hãng"],
                ["Activity", "Tư vấn da"],
                ["HeartHandshake", "Đồng hành"],
              ].map(([ic, t]) => {
                const II = Icon[ic];
                return (
                  <div key={t} className="flex flex-col items-center text-center gap-1.5">
                    <span className="text-brand-blue">
                      <II size={18} />
                    </span>
                    <span className="font-sans text-[9px] font-bold text-ink-2 uppercase tracking-wide">{t}</span>
                  </div>
                );
              })}
            </div>
  ...
  return (
    <div className="bg-white text-ink overflow-x-hidden selection:bg-brand-blue/20">
      ...
      <PageHero
        ...
        crumb={[["Sản phẩm", "/shop"], ["Giỏ hàng", null]]}
