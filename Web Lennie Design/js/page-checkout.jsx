/* ============================================================
   Thanh toán (Checkout) — shipping address → payment → success
   ============================================================ */
const CHECKOUT_PROVINCES = [
  'TP. Hồ Chí Minh', 'Hà Nội', 'Đà Nẵng', 'Hải Phòng', 'Cần Thơ',
  'Bình Dương', 'Đồng Nai', 'Bà Rịa - Vũng Tàu', 'Khánh Hòa', 'Lâm Đồng', 'Tỉnh / Thành khác',
];
...
function Field({ label, required, children, half }) {
  return (
    <div className={`space-y-1.5 ${half ? '' : 'sm:col-span-2'}`}>
      <label className="block font-sans text-[10px] font-extrabold tracking-widest text-ink-2 uppercase">{label}{required && '*'}</label>
      {children}
    </div>
  );
}
...
function StepDots({ step }) {
  const steps = [['1', 'Địa chỉ giao hàng'], ['2', 'Phương thức thanh toán'], ['3', 'Hoàn tất']];
  return (
...
function OrderSummaryCard({ items, count, subtotal, collapsible }) {
  ...
  return (
    <div className="bg-mist border border-divider rounded-md p-6 md:p-7 space-y-5">
      ...
      {collapsible && (
        <button type="button" onClick={() => setOpen(!open)} className="lg:hidden font-sans text-[10px] font-bold tracking-wider text-brand-blue uppercase">{open ? 'Thu gọn' : 'Xem sản phẩm'}</button>
      )}
      <div className="space-y-2.5 pt-4 border-t border-divider font-sans text-sm">
        <div className="flex justify-between text-ink-2"><span>Tạm tính</span><span className="font-semibold text-ink">{vnd(subtotal)}</span></div>
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg relative overflow-hidden modal-pop my-8">
        ...
        <div className="p-7 md:p-9 text-center">
          ...
          <div className="bg-mist border border-divider rounded-md p-5 mt-6 text-left space-y-3">
            ...
            <div className="flex justify-between font-sans text-[13px]"><span className="text-ink-2">Số sản phẩm</span><span className="text-ink font-semibold">{order.count}</span></div>
            <div className="flex justify-between font-sans text-[13px]"><span className="text-ink-2">Tổng tạm tính</span><span className="text-ink font-bold">{vnd(order.subtotal)}</span></div>
            <div className="flex justify-between font-sans text-[13px]"><span className="text-ink-2">Thanh toán</span><span className="text-ink font-semibold text-right">{order.paymentLabel}</span></div>
            <div className="flex justify-between gap-4 font-sans text-[13px]"><span className="text-ink-2 shrink-0">Giao tới</span><span className="text-ink font-medium text-right">{order.address}</span></div>
          </div>

          <div className="flex flex-col sm:flex-row gap-3 mt-7">
            <a href="index.html" className="flex-1 py-3.5 bg-brand-blue text-white font-sans text-[10px] font-bold tracking-[0.18em] uppercase rounded-sm hover:bg-ink transition-colors flex items-center justify-center">Về trang chủ</a>
            <a href="shop.html" className="flex-1 py-3.5 border border-brand-blue text-brand-blue font-sans text-[10px] font-bold tracking-[0.18em] uppercase rounded-sm hover:bg-brand-blue-light transition-colors flex items-center justify-center">Tiếp tục mua sắm</a>
          </div>
          <p className="font-sans text-[11px] text-ink-3 flex items-center justify-center gap-1.5 mt-5"><Icon.Phone size={13} className="text-brand-blue" />Hotline hỗ trợ: +84 909 123 456</p>
        </div>
      </div>
    </div>
  );
...
function CheckoutApp({ api }) {
  ...
  const goPayment = () => {
    setTouched(true);
    if (!step1Valid) { window.scrollTo({ top: 0, behavior: 'smooth' }); return; }
    setStep(1);
    ...
    return (
      <section className="bg-white py-20 md:py-28">
        <div className="max-w-2xl mx-auto px-6 text-center flex flex-col items-center gap-5">
          ...
          <a href="cart.html" className="mt-2 px-8 py-4 bg-ink text-white font-sans text-[10px] font-bold tracking-[0.2em] uppercase rounded-sm hover:bg-brand-blue transition-colors inline-flex items-center gap-2"><Icon.ChevronL size={15} />Quay lại giỏ hàng</a>
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="bg-white py-12 md:py-16">
      <div className="max-w-7xl mx-auto px-6">
        <div className="max-w-3xl mb-10"><StepDots step={order ? 2 : step} /></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-14">
          ...
          <div className="lg:col-span-7 xl:col-span-8 space-y-8">
            {step === 0 && (
              <div className="space-y-6">
                <div className="flex items-center gap-3 pb-4 border-b border-divider">
                  <span className="w-10 h-10 rounded-full bg-brand-blue-light flex items-center justify-center text-brand-blue"><Icon.Pin size={20} /></span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <Field label="Tỉnh / Thành phố" required half>
                    <select value={form.province} onChange={set('province')} className={`${inputCls} ${missing('province') ? 'border-red-300' : ''} ${!form.province ? 'text-ink-3/60' : ''}`}>
                      ...
                      {CHECKOUT_PROVINCES.map((p) => <option key={p} value={p} className="text-ink">{p}</option>)}
                    </select>
                  </Field>
                ...
                {touched && !step1Valid && <p className="font-sans text-[12px] text-red-500 flex items-center gap-1.5"><Icon.X size={14} />Vui lòng điền đầy đủ các trường bắt buộc (*).</p>}
                <div className="flex flex-col sm:flex-row gap-3 pt-2">
                  <button type="button" onClick={goPayment} className="px-8 py-4 bg-brand-blue text-white font-sans text-[10px] font-bold tracking-[0.2em] uppercase rounded-sm hover:bg-ink transition-colors flex items-center justify-center gap-2">Tiếp tục đến thanh toán<Icon.ArrowR size={15} /></button>
                  <a href="cart.html" className="px-7 py-4 border border-brand-blue text-brand-blue font-sans text-[10px] font-bold tracking-[0.18em] uppercase rounded-sm hover:bg-brand-blue-light transition-colors flex items-center justify-center gap-2"><Icon.ChevronL size={15} />Quay lại giỏ hàng</a>
                </div>
              </div>
            )}
            ...
            {step === 1 && (
              <div className="space-y-6">
                <div className="flex items-center gap-3 pb-4 border-b border-divider">
                  <span className="w-10 h-10 rounded-full bg-brand-blue-light flex items-center justify-center text-brand-blue"><Icon.Shield size={20} /></span>
                ...
                <div className="bg-brand-blue-light/60 border border-divider rounded-md p-5 flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3">
                    <span className="text-brand-blue shrink-0 mt-0.5"><Icon.Pin size={18} /></span>
                  ...
                  <button type="button" onClick={() => setStep(0)} className="shrink-0 font-sans text-[10px] font-bold tracking-wider text-brand-blue uppercase hover:text-ink">Sửa</button>
                </div>
                ...
                <div className="space-y-3">
                  {PAYMENT_METHODS.map((m) => {
                    ...
                    return (
                      <button key={m.id} type="button" onClick={() => setPayment(m.id)} className={`w-full text-left p-5 rounded-md border flex items-start gap-4 transition-all ${sel ? 'border-brand-blue bg-brand-blue-light ring-1 ring-brand-blue' : 'border-divider hover:border-brand-blue/50 bg-white'}`}>
                        <span className={`w-11 h-11 rounded-full flex items-center justify-center shrink-0 ${sel ? 'bg-brand-blue text-white' : 'bg-light-bg text-brand-blue'}`}><II size={20} /></span>
                ...
                <div className="bg-mist border border-divider rounded-md p-4 flex items-start gap-3">
                  ...
                  <p className="font-sans text-[12px] text-ink-2 leading-relaxed">Dù chọn phương thức nào, <strong className="text-ink">nhân viên Lennie</strong> luôn liên hệ trực tiếp với bạn để xác nhận đơn và hướng dẫn hoàn tất thanh toán cuối cùng một cách an toàn, minh bạch.</p>
                </div>

                <div className="flex flex-col sm:flex-row gap-3 pt-2">
                  <button type="button" onClick={placeOrder} className="px-9 py-4 bg-brand-blue text-white font-sans text-[10px] font-bold tracking-[0.2em] uppercase rounded-sm hover:bg-ink transition-colors flex items-center justify-center gap-2"><Icon.Check size={16} />Đặt hàng</button>
                  <button type="button" onClick={() => { setStep(0); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="px-7 py-4 border border-brand-blue text-brand-blue font-sans text-[10px] font-bold tracking-[0.18em] uppercase rounded-sm hover:bg-brand-blue-light transition-colors flex items-center justify-center gap-2"><Icon.ChevronL size={15} />Quay lại</button>
                </div>
              </div>
            )}
          </div>
      ...
      {(api) => (
        <>
          <PageHero
            ...
            crumb={[['Sản phẩm', 'shop.html'], ['Giỏ hàng', 'cart.html'], ['Thanh toán']]}
...
if (!window.__SPA) ReactDOM.createRoot(document.getElementById('root')).render(<CheckoutPage />);
